 ...
 * DesignToolbar
 *
 * Floating toolbar that appears when a design layer is selected.
 * Rendered inside the R3F scene via @react-three/drei Html component
 * so it automatically projects to 3D world position.
 *
 * Position is derived from the layer's UV coordinates converted to
 * world-space position on the back/front mesh surface.
 */
...
// ─── Toolbar inner (pure React, no Three.js) ─────────────────────────────────
...
// Converts UV [0,1] to a 3D position on a flat approximation of the mesh.
// For the back panel: positioned slightly behind the jersey in Z.

function uvToWorldBack(x: number, y: number): THREE.Vector3 {
  // Back mesh is roughly: X from -0.35 to 0.35, Y from 0.7 to 1.5, Z ≈ -0.07
...
// ─── Main exported component (rendered inside R3F Canvas) ─────────────────────
...
export const DesignToolbar = () => {
  ...
  // Toolbar floats slightly above the text (shift up in world space)

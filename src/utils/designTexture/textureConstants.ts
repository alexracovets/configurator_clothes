...
export function layerToAtlasUV(lx: number, ly: number, zone: PrintZoneKey): { u: number; v: number } {
  const b = UV0_BOUNDS[zone];
  return { u: b.minX + lx * (b.maxX - b.minX), v: b.minY + ly * (b.maxY - b.minY) };
}

export function atlasUVToLayer(u: number, v: number, zone: PrintZoneKey): { lx: number; ly: number } {
  const b = UV0_BOUNDS[zone];

...
const invalidateMeshCache = (): void => { _cachedScene = null; };
...
const getHitInZone = (e: PointerEvent, gl: THREE.WebGLRenderer, camera: THREE.Camera, scene: THREE.Scene, zone: PrintZoneKey): { uv: THREE.Vector2; zone: PrintZoneKey } | null => {
  const rect = gl.domElement.getBoundingClientRect();
  _ndc.set(((e.clientX - rect.left) / rect.width) * 2 - 1, ((e.clientY - rect.top) / rect.height) * -2 + 1);
  _ray.setFromCamera(_ndc, camera);
...
const getFrontSurfaceHit = (e: PointerEvent, gl: THREE.WebGLRenderer, camera: THREE.Camera, scene: THREE.Scene): { uv: THREE.Vector2; zone: PrintZoneKey } | null => {
  const hits = getHits(e, gl, camera, scene);
  return hits[0] ?? null;
};

export { zoneFromName, zoneFromUV, normaliseUV, invalidateMeshCache, getHits, getHitInZone, atlasHitTest, findLayerHit, getFrontSurfaceHit };

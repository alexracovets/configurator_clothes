...
const NameLayer = () => {
  ...
  useEffect(() => {
    ...
    const pickInstance = (clientX: number, clientY: number) => {
      ...
      const layout = buildNameDecalLayout(ctx, inst.text, inst.font, inst.fontSize);
      const zone = hitTestNameDecal(best.uv, layout);
    ...
    const applyDrag = (e: PointerEvent) => {
      ...
      if (drag.mode === "resize") {
        const width = Math.min(
          NAME_DECAL_SCALE_MAX,
          Math.max(NAME_DECAL_SCALE_MIN, drag.startScale + (dx - dy) * SCALE_SENS),
        );
        updateInstance(id, { decalScale: clampNameDecalScale(width) });
      }
    };

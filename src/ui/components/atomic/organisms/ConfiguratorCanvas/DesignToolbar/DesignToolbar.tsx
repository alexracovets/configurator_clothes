...
const ToolBtn = ({
  ...
  <button
    ...
    onMouseEnter={(e) => {
      (e.currentTarget as HTMLButtonElement).style.background = danger ? 'rgba(239,68,68,0.1)' : 'rgba(0,0,0,0.06)';
    }}
    onMouseLeave={(e) => {
...
const uvToWorldBack = (x: number, y: number): THREE.Vector3 => {
  return new THREE.Vector3((x - 0.5) * 0.7, 1.5 - y * 0.8, -0.07);
};
...
const DesignToolbar = () => {
  ...
  const layer = useConfiguratorStore((s) => (s.selectedId ? (s.layers.find((l) => l.id === s.selectedId) ?? null) : null));

  if (!layer || !selectedId) return null;

  const pos = layer.zone === 'front' ? uvToWorldFront(layer.x, layer.y) : uvToWorldBack(layer.x, layer.y);

  const toolbarPos = pos.clone().add(new THREE.Vector3(0, 0.12, 0));

  return (
    <Html position={[toolbarPos.x, toolbarPos.y, toolbarPos.z]} center zIndexRange={[100, 200]} style={{ pointerEvents: 'none' }} occlude={false}>

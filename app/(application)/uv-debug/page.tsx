...
declare namespace THREE {
  class Mesh { geometry: BufferGeometry; name: string; }
...
function UVSection() {
  ...
  const ws = (w: number): React.CSSProperties => ({
    display: "block", width: w, height: w, border: "1px solid #333",
  });
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 14, flexWrap: "wrap" }}>
        {legend.map(({ name, color }) => (
          <div key={name} style={{ display: "flex", alignItems: "center", gap: 5 }}>
            <div style={{ width: 11, height: 11, background: color, borderRadius: 2 }} />
...
// ── Texture Inspector ─────────────────────────────────────────────────────────
const TEXTURES = [
  { url: "/models/pbr/bake_ao-bake_roughness.jpg", label: "bake_ao-bake_roughness" },
  { url: "/models/pbr/bake_normal.jpg",            label: "bake_normal" },
  { url: "/models/pbr/cotton_jersey_nor_gl.jpg",   label: "cotton_jersey_nor_gl" },
...
function TexChannels({ url, label }: { url: string; label: string }) {
    ...
    img.onload = () => {
      ...
      refs.forEach((ref, ri) => {
        ...
        for (let y = 0; y < S; y++) for (let x = 0; x < S; x++) {
          ...
          const di = (y * S + x) * 4;
          if (ch === -1) {
            id.data[di] = data[si]; id.data[di+1] = data[si+1]; id.data[di+2] = data[si+2];
          } else {
  ...
  const cs: React.CSSProperties = { width: 256, height: 256, display: "block", border: "1px solid #333" };
  const labels = ["R channel", "G channel", "B channel", "Full"];
  const colors  = ["#f88", "#8f8", "#88f", "#aaa"];

  return (
    <div style={{ background: "#161616", borderRadius: 8, padding: 14, marginBottom: 20 }}>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        {labels.map((lbl, i) => (
          <div key={lbl} style={{ textAlign: "center" }}>
            <div style={{ fontSize: 11, color: colors[i], marginBottom: 4 }}>{lbl}</div>
            <canvas ref={refs[i]} style={cs} />
          </div>
        ))}
      </div>
    </div>
...
export default function DebugPage() {
  ...
  return (
    <div style={{ background: "#0a0a0a", minHeight: "100vh", padding: 20, color: "#fff", fontFamily: "monospace" }}>
      ...
      {tab === "tex" && (
        <section>
          ...
          {TEXTURES.map(t => <TexChannels key={t.url} {...t} />)}
        </section>
      )}
    </div>
  );

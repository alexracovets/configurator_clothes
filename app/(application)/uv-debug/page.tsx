...
function UVSection() {
      ...
      new GLTFLoader().load(MODEL_PATH, (gltf) => {
        ...
        if (c0 && c1) {
          ...
          gltf.scene.traverse((obj: unknown) => {
            ...
            const uv0 = geo.attributes["uv"] as
              | THREE.BufferAttribute
              | undefined;
            const uv1 = geo.attributes["uv1"] as
              | THREE.BufferAttribute
              | undefined;
            const idx = geo.index;
  ...
  return (
    <section>
      <h2 style={{ fontSize: 16, marginBottom: 8, color: "#ffd" }}>
        UV Channels — crewneck.gltf
      </h2>
      ...
      <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
        <div>
          <p style={{ marginBottom: 5, fontSize: 12, color: "#aaa" }}>
            UV0 — TEXCOORD_0
          </p>
...
function TexChannels({ url, label }: { url: string; label: string }) {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      setInfo(
        `${img.width}×${img.height}  R:${(sR / n).toFixed(1)}  G:${(sG / n).toFixed(1)}  B:${(sB / n).toFixed(1)}`,
      );
      const S = 256;
  ...
  return (
    ...
    >
      ...
      <div
        style={{ fontSize: 11, color: "#888", marginBottom: 10, marginTop: 2 }}
      >
        {info}
      </div>
...
export default function DebugPage() {
  ...
  return (
    ...
    >
      ...
      {tab === "tex" && (
        <section>
          <h2 style={{ fontSize: 16, marginBottom: 14, color: "#ffd" }}>
